...
\begin{enumerate}
  ...
  \item \textbf{Empirical pilot (cautious).} In an initial set of ten
    ...
    independent analyses (Section~\ref{sec:results}). A scaled
    instance with 424 baseline questions then revises two of the
    small-sample baseline conclusions---engagement discriminates at
    scale, and premise refutation is rare (0--3.2\%) but not unique to
    the evidence-graph system---demonstrating that the benchmark
    updates its own conclusions as $n$ grows
    (Sections~\ref{sec:baselines} and~\ref{sec:scaled}). A generator
    decomposition and a four-cutoff temporal stress test then separate
    memorization from foresight empirically: the foresight signal
    survives in a weight-free structural generator at every cutoff,
    while LLM-only generation shows relevance without resolution in
    every era (Section~\ref{sec:foresight}). With $n=10$
